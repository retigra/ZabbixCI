...
class TestGlobalMacroBlacklist(BaseGlobalMacro, unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        super().setUp()
        Settings.GLOBAL_MACRO_BLACKLIST = (
            "{$NOT_EXISTING_MACRO},{$NOT_EXISTING_MACRO_2}"
        )

    # Test global macro deletion with blacklist checks
    async def test_global_macro_delete(self):
        ...
        Settings.GLOBAL_MACRO_BLACKLIST = "{$TEST_MACRO}"

        # Push changes to git
        changed = await self.zci.push()
        self.assertFalse(changed, "Global macro deletion detected inside of blacklist")
