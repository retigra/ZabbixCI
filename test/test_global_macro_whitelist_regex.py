...
class TestGlobalMacroWhitelistRegex(BaseGlobalMacro, unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        Settings.GLOBAL_MACRO_WHITELIST = r"{\$TEST_.*},{\$ANOTHER_.*}"

    # Test global macro deletion with regex whitelist checks
    async def test_global_macro_delete(self):
        ...
        Settings.GLOBAL_MACRO_WHITELIST = r"{\$NONEXISTENT_.*}"

        # Push changes to git
        changed = await self.zci.push()
        self.assertFalse(
            changed, "Global macro deletion detected outside of regex whitelist"
        )
